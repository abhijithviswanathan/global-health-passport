...
## Local photo face-presence check
The photo revision adds one non-clinical machine-learning use: the bundled OpenCV YuNet model rejects photos without exactly one sufficiently visible face. It runs on the API host without external image requests. It does not recognize identity, infer demographics, establish liveness or provide medical advice. It must not be treated as patient identity verification. False acceptance/rejection and demographic performance have not been comprehensively evaluated. Failed/unavailable checks block the upload, while adding a photo remains optional. The test portrait is AI-generated and passes deliberately, demonstrating that this is not a liveness check.
