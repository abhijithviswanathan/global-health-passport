#!/usr/bin/env python3
# Builds the illustrated showcase report from fictional fixture/evidence files.
# Regenerate the report after changing those examples; it does not start the app.
...
teal = colors.HexColor("#146B5D")
ink = colors.HexColor("#20373A")
muted = colors.HexColor("#587074")
pale = colors.HexColor("#EEF5F2")
gold = colors.HexColor("#E7A24E")
...
styles.add(
    ParagraphStyle(
        name="CellCustom", fontName="Helvetica", fontSize=8.5, leading=12, textColor=ink
    )
)
story = []
...
def p(x, style="BodyCustom"):
    return Paragraph(escape(clean(x)).replace("\n", "<br/>"), styles[style])


def add(x, style="BodyCustom"):
    story.append(p(x, style))
...
def page(title, subtitle):
    ...
    add(title, "TitleCustom")
    add(subtitle, "Sub")


def table(headers, rows, widths):
    data = [[p(x, "CellCustom") for x in headers]] + [
        [p(x, "CellCustom") for x in row] for row in rows
    ]
    ...
    t.setStyle(
        TableStyle(
            [
                ...
                ("LINEBELOW", (0, 1), (-1, -1), 0.3, colors.HexColor("#D9E5E2")),
            ]
        )
    )
    story.append(t)
...
def time(a):
    return (
        datetime.fromtimestamp(a["starts_at"] / 1000, ZoneInfo("America/New_York"))
        .strftime("%I:%M %p")
        .lstrip("0")
    )
...
def case(c):
    ...
    add(c["story"], "Sub")
    add(
        "Health ID " + c["healthId"] + " | Login " + c["username"] + " | " + c["room"],
        "SmallCustom",
    )
    add(c["history"])
    add("Allergy / intolerance context: " + c["allergy"])
    z = c["vitals"]
    add(
        f"Invented intake: {z['temperature_c']} C; SpO2 {z['spo2_percent']}%; pulse {z['pulse_bpm']}/min; BP {z['systolic_mmhg']}/{z['diastolic_mmhg']} mmHg."
    )
    add("Assessment context: " + c["condition"])
    add("Next step: " + c["plan"])


page(
    "Health Passport\nA hospital day, explained",
    "Fictional demonstration pack | NorthStar Hospital | 11 September 2026",
)
add(
    "Use four connected patient stories to show how a doctor, nurse, laboratory, imaging team and patient work from the same saved information. Names, histories, results, addresses and insurance examples are invented."
)
table(
    ["4 patients", "33 clinical entries", "5 requests", "4 visits"],
    [
        [
            "Alice, Noah, Fatima, Leo",
            "26 authored + 5 orders + 2 reports",
...
add(
    "Actual local web application: the doctor sees four authorized patients, short Health IDs and the next appointment. Screenshots reflect the saved demo on 11 September.",
    "SmallCustom",
)
section("Start here")
add(
    "Open http://localhost:5173/?portal=doctor and sign in as hospitaldoctor. Use the existing local demo password in LOCAL_ACCESS.txt. The generic doctor account belongs to a different organization. For the mobile browser preview, open http://localhost:5174/."
)

page(
    "Meet Alice and Noah",
    "One case has completed reports; the other makes pending work visible.",
)
case(m["patients"]["hospitalalice"])
case(m["patients"]["showcasenoah"])
add(
    "Noah also has a server-saved SOAP draft attached to his appointment. It is unfinished documentation, not a signed consultation.",
    "SmallCustom",
)

page(
    "Meet Fatima and Leo",
    "Two more stories demonstrate follow-up, medication reconciliation and imaging coordination.",
)
case(m["patients"]["showcasefatima"])
case(m["patients"]["showcaseleo"])
add(
    "Fatima has a prescription for the nonexistent DEMO TRAINING TABLET: 10 fictional units, with one demonstration dispensing unit recorded. No real medicine was supplied or recommended.",
    "SmallCustom",
)

page(
    "From request to report",
    "Alice demonstrates the completed laboratory and imaging journeys.",
)
table(
    ["Laboratory step", "Fictional evidence"],
    [
        [
            "Order and identity",
            "Alice CBC report, assigned to Technician Lee; patient and specimen identifiers recorded.",
        ],
        ["Result", "WBC 6.8 x10^9/L; haemoglobin 13.2 g/dL; platelets 254 x10^9/L."],
        [
            "Review and completion",
            "Result stored, Dr Smith review recorded, workflow completed.",
        ],
    ],
    [137, 367],
)
...
for r in sorted(v["reports"], key=lambda x: x["kind"]):
    add(r["title"], "SectionCustom")
    add(r["details"])
section("The date care happened is preserved")
add(
    "The sample observations are dated 10 September 2026, while their upload is dated 11 September 2026. The chart preserves the author, role, source, original observation timestamp and entry timestamp. It does not present the older measurement as a new reading."
)
add(
    "The chest study reference is metadata only. No actual X-ray or DICOM image pixels were created. Report language and laboratory values are invented demonstration content, without diagnostic guidance.",
    "SmallCustom",
)
section("What this demonstrates")
add(
    "An authorized patient can read the same result in the mobile timeline. Team members can inspect the request state, and the doctor can review the report without uploading it again. Nurse task completion has a separate doctor verification."
)

page(
    "Work that still needs attention",
    "Pending requests, named owners and explicit handoff states make the demonstration useful.",
)
table(
    ["Patient / request", "Saved state", "Owner"],
    [
        [
            "Alice / CBC and chest reports",
            "Both completed",
            "Technician Lee / Technician Patel; Dr Ahmed attestation; Dr Smith review",
        ],
        ["Noah / wellness panel", "Processing; no result", "Technician Lee"],
        ["Fatima / follow-up review", "Ordered", "Dr Smith"],
        ["Leo / left ankle imaging", "Scheduled; no result", "Technician Patel"],
    ],
    [162, 132, 210],
)
table(
    ["Nurse Williams task", "Saved state", "Location"],
    [
        [
            t["title"].replace("Showcase · ", ""),
            t["status"].replace("_", " ")
            + (" + doctor verified" if t.get("verified_by") else ""),
            t["location_label"],
        ]
...
add(
    "Three care-context conversations contain six messages. Alice: confirm access to reviewed reports. Noah: leave the result-review task open during processing. Leo: confirm room and time for imaging. Nurse Williams replies with an acknowledgment in each thread; the task must still be updated separately."
)
add(
    "Two structured nurse-to-doctor handoffs carry patient status, pending tasks, medication attention, tests, observations and concerns. Alice's handoff is acknowledged. Leo's handoff is sent and awaits acknowledgment."
)
add(
    "The five structured orders also create five linked tasks. Together with the four curated nurse tasks, this addition supplies nine tasks. Existing earlier demo records remain available.",
    "SmallCustom",
)

page(
    "Coverage and the demo cast",
    "Insurance comparisons and role-specific accounts complete the story.",
)
table(
    ["Invented plan", "Monthly premium", "Deductible", "Out-of-pocket", "Network"],
    [
        ["Harbor Starter", "$220", "$1,800", "$6,500", "HMO"],
        ["Harbor Flexible", "$365", "$900", "$4,500", "PPO"],
        ["Harbor Family", "$480", "$1,500", "$7,000", "EPO"],
    ],
    [144, 95, 85, 100, 80],
)
add(
    "All three Showcase plans are fictional approved listings in USD, with an illustrative $25 office copay and 20% coinsurance description. They are not offers, recommendations or enrollment options."
)
add(
    "Noah: Example Unconnected Provider - Fictional returns UNKNOWN. Fatima: Synthetic Harbor Health returns a synthetic VERIFIED response. Neither involved contacting a real insurer. Two patient-owned profiles have purpose-specific billing access."
)
section("Sign in to show each perspective")
table(
    ["Username", "Demo role / what to show"],
    [
        ["hospitaldoctor", "Dr Smith: patients, schedule, request review, Noah draft"],
        ["hospitalnurse", "Nurse Williams: work grid, observations and handoffs"],
        ["hospitallab", "Technician Lee: specimen and pending laboratory work"],
        [
            "hospitalimaging / hospitalradiologist",
            "Technician Patel / Dr Ahmed: imaging and report attestation",
        ],
        ["hospitalpharmacy", "Pharmacist Davis: fictional prescription and dispensing"],
        [
            "hospitalbilling / hospitalreception",
            "Billing Taylor / Reception Jordan: insurance or registration",
        ],
        [
            "hospitaladmin / hospitalinsurer",
            "Hospital organization / fictional plan management",
        ],
        [
            "hospitalalice / showcasenoah / showcasefatima / showcaseleo",
            "Each patient sees their own authorized records",
        ],
    ],
    [230, 274],
)
add(
    "Credentials stay in the local LOCAL_ACCESS.txt file and .env; this shareable report deliberately contains no passwords. Sign out before changing roles, or use separate browser profiles.",
    "SmallCustom",
)

page(
    "Your eight-minute presentation",
    "Follow this route to explain the value through a complete patient story.",
)
steps = [
    (
...
for title, body in steps:
    left.extend([p(title, "SectionCustom"), p(body)])
right = [
    photo("showcase-native-timeline.png", 167, 398),
    p(
        "Mobile timeline / actual React Native browser harness. Physical iOS and Android devices were not tested in this revision.",
        "SmallCustom",
    ),
]
t = Table([[left, right]], colWidths=[323, 181])
t.setStyle(
    TableStyle(
        [
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
add(
    "PASS: four focused browser checks - web doctor directory/orders; web nurse work grid; native-code patient reports/marketplace; native-code nurse work grid. Screenshots and machine-readable results are saved with the project. These are browser checks, not physical-device tests."
)
section("Where everything is saved")
add(
    "Desktop > Health Passport opens the local project. This report is in output/pdf. DEMO_SHOWCASE_REPORT.md contains the full inventory, exact examples, messages and run instructions. docs/evidence/showcase-demo.json records IDs and states; showcase-verification.json and showcase-ui.json record the checks. The scripts are in scripts/. No public deployment was made."
)
section("Run and maintain the demonstration")
add(
    "Use Start Health Passport.command in the project to start the local app. Keep the local database, .env and encryption/signing material together according to the existing backup guide. With the server running in DEMO_MODE, the Showcase setup script reuses stable request keys and preserves staged progress. This is a fixed 11 September snapshot: select that date in the agenda later. Existing README files were preserved in this revision."
)
add(
    "Not established by this demo: real insurer/EHR/PACS integration, actual image acquisition, external message delivery, physical-device camera/push behavior or readiness for real patient care. The exact remaining release work is retained in the earlier expansion report.",
    "SmallCustom",
)
...
def furniture(c, doc):
    ...
    w, h = doc.pagesize
    c.setFillColor(teal)
    c.rect(44, h - 28, 28, 4, fill=1, stroke=0)
    c.setFont("Helvetica-Bold", 8)
    c.drawString(80, h - 27, "HEALTH PASSPORT / DEMONSTRATION REPORT")
    c.setStrokeColor(colors.HexColor("#D9E5E2"))
    c.line(44, 42, w - 44, 42)
    c.setFont("Helvetica", 7.5)
    c.setFillColor(muted)
    c.drawString(44, 28, "Fictional data only  |  Local project  |  11 September 2026")
    c.drawRightString(w - 44, 28, str(doc.page))
    c.restoreState()
...
SimpleDocTemplate(
    ...
    topMargin=55,
    bottomMargin=57,
    title="Health Passport - Fictional Demonstration Report",
    author="Health Passport project",
...
md = [
    ...
    "- Schedule snapshot: "
    + m["scheduleDate"]
    + " (America/New_York). Later, select this date in the agenda.",
    "",
    "## Everything added",
    "",
    "| Item | Count |",
...
for c in m["patients"].values():
    md += [
        "### " + c["name"],
        "",
        f"- Login: `{c['username']}`; Health ID: `{c['healthId']}`; age: {c['age']}; occupation: {c['occupation']}.",
        f"- Appointment: {m['scheduleDate']} at {time(m['appointments'][c['username']])} Eastern, 30 minutes, in person.",
        "- Location: " + c["room"] + ".",
        "- Presenting concern: " + c["story"] + ".",
        "",
        c["history"],
        "",
        "Allergy / intolerance: " + c["allergy"],
        "",
        "Assessment context: " + c["condition"],
        "",
        "Invented vitals: " + json.dumps(c["vitals"]) + ".",
        "",
        "Follow-up: " + c["plan"],
        "",
    ]
...
for r in sorted(v["reports"], key=lambda x: x["kind"]):
    md += [
        ...
        r["details"],
        "",
        f"Author: {r['author_name']} ({r['author_role']}). Observed: {r['observed_at']}. Entered: {r['created_at']}. Related order record: `{r['related_id']}`.",
        "",
    ]
md += ["## Requests and tasks", "", "| Request | State | Result |", "|---|---|---|"]
for o in m["orders"].values():
    md += [
        f"| {o['code']} | {o['status']} | {'Saved report' if o['result_id'] else 'None - pending'} |"
    ]
md += [
    "",
...
for t in m["tasks"].values():
    md += [
        f"| {t['title']} | {t['status']}"
        + (" / independently verified" if t.get("verified_by") else "")
        + f" | {t['location_label']} |"
    ]
md += [
    "",
...
for msg in v["messagesContent"]:
    md += ["- **" + msg["author_name"] + "**: " + msg["body"]]
md += ["", "## Structured handoffs", ""]
for key, h in m["handoffs"].items():
    md += ["### " + key + " - " + h["status"], "", h["details"], ""]
md += [
...
for key, rid in m["records"].items():
    md += [f"| {key} | `{rid}` |"]
