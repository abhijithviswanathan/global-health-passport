...
An Expo / React Native patient application for the synthetic pilot. It connects to the Java service used by the web portal. This is source code plus verified JavaScript bundles, not a signed App Store or Play Store release.
...
Use the synthetic patient credentials in the root README. Staff accounts are rejected by this patient application. For a native development build:
...
Network cookies are managed by the native fetch stack. A failed network logout clears local screen state but cannot guarantee remote session invalidation until connectivity returns or the session expires. Device-record synchronization, notifications, camera uploads, QR code and distribution signing remain incomplete. Biometric unlock is not server-side MFA, and the privacy screen does not establish prevention of OS screenshots or extraction on a compromised device.
...
- Eight tests: passed (four emergency-policy tests plus four API transport tests). Emergency tests cover TTL, scope/count/byte limits, clock reversal, text preservation, and exclusion of unrelated fields. API transport checks: passed (fresh CSRF, cookie/no-store options, backend-field normalization, cleartext rejection in release, and preserved unauthorized status are covered across the four cases).
...
2. Verify login, fresh CSRF, expiry, logout, and role rejection against the deployed service on each platform.
