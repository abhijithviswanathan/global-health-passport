...
To run beside another local checkout, set `PASSPORT_TEST_WEB_URL` and `PASSPORT_TEST_MOBILE_URL` consistently for the test process, and set `PASSPORT_TEST_MOBILE_URL` for the Vite harness as well. For example, use `http://localhost:5193` for the full web server and `http://localhost:5194` for the native harness. The harness derives its listening port and request origin from that URL. Both proxies still expect the local Java API on port 8080. These variables affect test infrastructure only, not release app configuration.
