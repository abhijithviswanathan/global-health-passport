...
import { createTypeScriptLoader } from "../../../scripts/tests/load-typescript.mjs";
import { test } from "node:test";
import ts from "typescript";
const api = await createTypeScriptLoader(ts)(
  ...
  return {
    ...
    text: async () =>
      JSON.stringify(
        url.endsWith("/csrf")
          ? { token: "fresh-csrf" }
          : [{ created_at: "2026-09-10", grantee_name: "Care team" }],
      ),
  };
};
...
test("normalizes single appointment and draft responses like list responses", async () => {
  ...
  assert.deepEqual(await api.request("/clinician/appointments/a"), {
    patientId: "p",
    startsAt: 123,
    durationMinutes: 30,
    updatedAt: "now",
  ...
  globalThis.fetch = async (url) =>
    url.endsWith("/csrf")
      ? {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ token: "csrf" }),
        }
      : {
          ok: false,
          status: 409,
          text: async () =>
            JSON.stringify({
              message: "Visit changed. Refresh and review the latest note.",
            }),
        };
  await assert.rejects(
    api.request("/clinician/appointments/a/draft", "PUT", {}),
    (e) => e.status === 409 && e.message.includes("Visit changed"),
  );
});
...
test("photo upload preserves multipart boundaries and sends CSRF with cookies", async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    return {
      ...
      text: async () =>
        JSON.stringify(
          url.endsWith("/csrf") ? { token: "photo-csrf" } : { id: "photo-id" },
        ),
    };
  };
...
test("shared workspaces can retain raw rows without recursively renaming nested data", async () => {
  const row = {
    patient_id: "synthetic-patient",
    measurements: { pulse_bpm: 72 },
  };
  globalThis.fetch = async () => ({
    ok: true,
    status: 200,
  ...
  assert.deepEqual(
    await api.request("/care/records", "GET", undefined, 15000, true),
    row,
  );
  assert.deepEqual(await api.request("/care/records"), {
